...
public class CVUIBeanShell extends JPanel {
	 ...
	 */
	private final String lk_szConfigTokenPath = "CVUIBeanShellScriptfilePath";
	...
	private JButton jButtonSaveScriptAs = null;
	 ...
	 */
	public CVUIBeanShell(ValentinConsole valentinConsole, CVErrorMessage cErrorMessage, CVLogging cErrorFile,
			CVStatusLine cStatusMessage, CVConfigFile cConfigFile, CVConnectionManager cConnectionManager) {
		...
		this.lk_cBeanShell = new CVBeanShell(this.lk_valentinConsole, this.lk_cErrorMessage, this.lk_cErrorFile,
				this.lk_cStatusMessage, this.lk_cConfigFile);

		this.lk_cBeanShellEditor = new CVUIBeanShellEditor(this.lk_cErrorMessage, this.lk_cErrorFile,
				this.lk_cStatusMessage, this.lk_cConfigFile);

		this.lk_cFileManagement = new CVFileManagement(this.lk_cErrorMessage, this.lk_cErrorFile,
				this.lk_cStatusMessage);

		this.lk_cDisplayInterfaceASCII = new CVTextDisplay(this.lk_cErrorMessage, this.lk_cErrorFile,
				this.lk_cStatusMessage);

		this.lk_cDisplayInterfaceHEX = new CVTextDisplay(this.lk_cErrorMessage, this.lk_cErrorFile,
				this.lk_cStatusMessage);
	...
	private void setupBeanShellInterpreter() {
		...
		this.lk_cBeanShell.setDisplayInterfaceASCII(this.lk_cDisplayInterfaceASCII);
		this.lk_cBeanShell.setDisplayInterfaceHEX(this.lk_cDisplayInterfaceHEX);
	...
	private void processButtonLoadScript() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			absoluteScriptPath = cScriptFile.getAbsolutePath();

			// ------------------------------------------------------------------
			// Speichern gewaehlter Pfad in Konfigurationsdatei
			// ------------------------------------------------------------------
			if (this.lk_cConfigFile != null) {
				lk_cConfigFile.setConfig(this.lk_szConfigTokenPath, cScriptFile.getAbsolutePath());
			}
	 ...
	 */
	private void processButtonSaveScriptAs() {
		final JFileChooser cFileChooser = new JFileChooser();
	...
	private void processButtonSaveScript() {
		...
		if (absoluteScriptPath != null && !absoluteScriptPath.equals("")) {
			scriptFile = new File(absoluteScriptPath);
			if (scriptFile != null) {
				// ------------------------------------------------------------------
				// Speichern gewaehlter Pfad in Konfigurationsdatei
				// ------------------------------------------------------------------
				if (this.lk_cConfigFile != null) {
					lk_cConfigFile.setConfig(this.lk_szConfigTokenPath, scriptFile.getAbsolutePath());
				}

				if (true == this.lk_cBeanShellEditor.storeScriptFile(scriptFile)) {
					this.jTextFieldCurrentScript.setText(scriptFile.getName());
				}
			}
		}
		return;
	...
	private void processButtonSaveOutput() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			// ------------------------------------------------------------------
			if (this.lk_cConfigFile != null) {
				lk_cConfigFile.setConfig(this.lk_szConfigTokenPath, cOutputFile.getAbsolutePath());
			}

			this.lk_cFileManagement.storeStringToFile(cOutputFile, this.jTextAreaScriptOutput.getText());
		}
	...
	public void processButtonRunScript(boolean bAutoScript) {
		...
		this.lk_cBeanShell.runScript(lk_cBeanShellEditor.getScriptFile(), bAutoScript);
	...
	private void processButtonFind() {
		...
		replaceButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				replace(false, replaceField.getText());
			}
		});
		...
		replaceAllButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				replace(true, replaceField.getText());
			}
		});
	...
	/**
	 * This method replaces searched Strings in Rsyntaxtextarea
	 *
	 * @return void
	 */
	private void replace(Boolean replaceAll, String replaceString) {
		SearchContext context = new SearchContext();
		String text = searchField.getText();
		if (text.length() != 0 && replaceString.length() != 0) {
			context.setSearchFor(searchField.getText());
			context.setReplaceWith(replaceString);
			if (replaceAll) {
				SearchEngine.replaceAll(lk_cBeanShellEditor, context);
			} else {
				SearchEngine.replace(lk_cBeanShellEditor, context);
			}
		} else {
			JOptionPane.showMessageDialog(lk_valentinConsole, "Please insert Text");
		}
	}
	...
	private JPanel getJPanelScriptButtonBarTop() {
		if (jPanelScriptButtonBarTop == null) {
			...
			jPanelScriptButtonBarTop.add(getJButtonSaveScriptAs(), null);
	...
	private JTabbedPane getJTabbedPaneScript() {
		if (jTabbedPaneScript == null) {
			...
			jTabbedPaneScript.addTab("ScriptEditor", null, getJScrollPaneScriptEditor(), "view and modify scripts");
			jTabbedPaneScript.addTab("Script Output", null, getJScrollPaneScriptOutput(), "output current script");
			jTabbedPaneScript.addTab("Interface ASCII", null, this.lk_cDisplayInterfaceASCII.getTextAreaPane(),
					"interface data ASCII formatted");
			/*
			 * jTabbedPaneScript.addTab( "Interface HEX", null,
	...
	private JPanel getJPanelScriptButtonBarBottom() {
		if (jPanelScriptButtonBarBottom == null) {
			...
			jPanelScriptButtonBarBottom.add(getJCheckBoxShowErrorMessages(), null);
	 ...
	 * @return javax.swing.JButton
	 */
	private JButton getJButtonSaveScriptAs() {
		if (jButtonSaveScriptAs == null) {
			jButtonSaveScriptAs = new JButton();
			jButtonSaveScriptAs.setText("Save Script As");
			jButtonSaveScriptAs.setToolTipText("save script from editor to file");
			jButtonSaveScriptAs.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					processButtonSaveScriptAs();
				}
			});
		}
		return jButtonSaveScriptAs;
	}
	...
	private JTextField getJTextFieldCurrentScript() {
		if (jTextFieldCurrentScript == null) {
			...
			jTextFieldCurrentScript.setPreferredSize(new java.awt.Dimension(200, 20));
		}
	...
	private JButton getJButtonRunScript() {
		if (jButtonRunScript == null) {
			...
			jButtonRunScript.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
	...
	private JButton getJButtonStopScript() {
		if (jButtonStopScript == null) {
			...
			jButtonStopScript.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
	...
	private JButton getJButtonStepScript() {
		if (jButtonStepScript == null) {
			...
			jButtonStepScript.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
	...
	private JTextArea getJTextAreaScriptOutput() {
		if (jTextAreaScriptOutput == null) {
			...
			jTextAreaScriptOutput.setFont(new java.awt.Font("DialogInput", java.awt.Font.PLAIN, 12));
			jTextAreaScriptOutput.getDocument().addDocumentListener(new documentListenerJTA() {
				public void insertUpdate(DocumentEvent e) {
					if (!lk_bStdOutPrintf) {
						return;
					}
					try {
						System.out.print(e.getDocument().getText(e.getOffset(), e.getLength()));
	...
	private JCheckBox getJCheckBoxShowErrorMessages() {
		if (jCheckBoxShowErrorMessages == null) {
			...
			jCheckBoxShowErrorMessages.setToolTipText("enable or disable separate windows with error messages");
			jCheckBoxShowErrorMessages.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					processButoonEnableErrorMessages(jCheckBoxShowErrorMessages.isSelected());
				}
			});
		}
