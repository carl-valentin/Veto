...
public class CVCommandFile
{
    ...
    public CVCommandFile(String strName)
    {
        ...
        try
        {
            FileReader fileConfig =
                new FileReader(strConfigFileName);
            ...
            while ( (i = fileConfig.read()) > 0 )
            {
                ...
                {
                    Command_Element cfgElement =
                        new Command_Element(strID, inBuf.toString());
                    list.add(cfgElement);
                    inBuf = new StringBuffer();
                }
                else
    ...
    public boolean setConfig(String strID, String strValue)
    {
        ...
        Command_Element cfgElement =
            new Command_Element(strID, strValue);
        list.add(cfgElement);
        writeConfig();
        return true;
    }
    ...
    private void writeConfig()
    {
        try
        {
            FileWriter fileConfig =
                new FileWriter(strConfigFileName);
            Object objElement[] = list.toArray();
            for (int i=0; i<list.size(); i++)
            {
                Command_Element cfgElement = (Command_Element)objElement[i];
